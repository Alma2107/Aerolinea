...
$equipajes_elegidos = $_SESSION['equipajes_elegidos'] ?? [];
...
if ($id_vuelo <= 0 || $id_plan <= 0 || $id_asiento <= 0 || empty($datos_pasajeros)) {
    ...
    echo '<p>Volve al flujo de compra y completa vuelo, equipaje, asiento y pasajeros antes de finalizar el pago.</p>';
...
try {
    ...
    foreach ($datos_pasajeros as $pas) {
        ...
        foreach ($equipajes_elegidos as $id_equipaje) {
            $stmtPrecioEq = $pdo->prepare("SELECT precio_unitario FROM tipos_equipaje WHERE id_tipo_equipaje = :id LIMIT 1");
            $stmtPrecioEq->execute(['id' => $id_equipaje]);
            $precioEquipaje = (float)($stmtPrecioEq->fetchColumn() ?: 0);
            if (!empty($_SESSION['codigo_promo']) && $_SESSION['codigo_promo'] === 'EQUIPAJEGRATIS') {
                $precioEquipaje = 0;
            }
            $stmtEq = $pdo->prepare("INSERT INTO ticket_equipajes (id_ticket, id_tipo_equipaje, cantidad, precio_pagado) VALUES (:id_ticket, :id_equipaje, 1, :precio)");
            $stmtEq->execute(['id_ticket' => $id_ticket, 'id_equipaje' => $id_equipaje, 'precio' => $precioEquipaje]);
        }
    ...
    unset($_SESSION['id_vuelo'], $_SESSION['id_plan'], $_SESSION['id_asiento'], $_SESSION['datos_pasajeros'], $_SESSION['servicios_elegidos'], $_SESSION['equipajes_elegidos'], $_SESSION['promo_equipaje_gratis']);

    $primerPnr = $pnrsGenerados[0] ?? '';
    $pasajeUrl = 'http://' . $_SERVER['HTTP_HOST'] . '/Aerolinea/consultas/reservas/pasaje.php?pnr=' . urlencode($primerPnr);
    ...
    echo '<p style="margin:15px 0;">Codigo PNR: <strong>' . htmlspecialchars($primerPnr, ENT_QUOTES, 'UTF-8') . '</strong></p>';
    echo '<img src="' . htmlspecialchars($qrUrl, ENT_QUOTES, 'UTF-8') . '" alt="QR del pasaje" style="width:180px; height:180px; padding:8px; border-radius:16px; background:#fff4df; margin:10px auto;">';
    echo '<p style="margin:15px 0;">Escanea el QR o abri tu pasaje virtual para descargarlo como PDF desde imprimir.</p>';
    ...
    echo '<p>Se produjo un error durante la confirmacion. Revisa que el flujo anterior este completo e intenta de nuevo.</p>';
